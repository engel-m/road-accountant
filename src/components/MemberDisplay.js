...
const MemberCard = React.memo(props => {
  ...
  return (      
    <div className="relative flex w-1/3 md:w-1/5 flex-col py-4 my-2 items-center bg-white shadow-lg rounded flex-shrink mx-1">
      {props.member.nickname === 'Mike' && <p className="absolute top-0 left-0 mt-1 ml-2 text-xs md:text-base text-green-600 font-bold">Creator</p>}
      ...
      <h1 aria-label={props.member.email} data-balloon-pos="down" className={"tooltip-grey font-bold text-xl md:text-2xl my-auto text-" + color + ""}>{props.member.nickname}</h1>
    </div>
  );
});
...
export const MemberDisplay = () => {
  ...
  return (
    <>
    <h4 className="font-bold text-xl text-center">Group Members</h4>
    <div className="w-full max-w-6xl mx-auto mt-6 flex flex-wrap flex-column justify-center">      
        ...
        {/* Add Button */}
        <div onClick={toggleAddModal} className="flex cursor-pointer w-1/3 md:w-1/5 flex-col py-2 md:py-4 my-2 
